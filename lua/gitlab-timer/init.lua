...
function M.run_tests()
	...
	assert_eq(
		extract_project_from_url("https://gitlab.com/group/project/-/issues/1"),
		"group/project",
		"extract_project_from_url"
	)
...
function M.setup(opts)
	...
	if __url == "" or not __url:match("^https?://") then
		notify(
			"gitlab_url seems invalid. Set a proper URL like https://gitlab.com or your self-hosted instance.",
			vim.log.levels.WARN
		)
	end
				...
				vim.schedule(function()
					if err then
						...
						for _, issue in ipairs(data or {}) do
							...
							local project_path = extract_project_from_url(issue.webUrl or "")
							issue.project = {
								id = proj_meta and proj_meta.id
									or (
										"gid://gitlab/Project/"
										.. (issue.project_id and tostring(issue.project_id) or "")
									),
								name = (proj_meta and proj_meta.name)
									or (project_path:match("/([^/]+)$") or project_path or "Unknown"),
								fullPath = (proj_meta and proj_meta.fullPath) or (project_path or "Unknown"),
								nameWithNamespace = (proj_meta and proj_meta.nameWithNamespace)
									or (project_path or "Unknown"),
							}
						end
						cb(nil, data or {})
					end
...
function M.add_time_to_issue_async(issue, time_spent, message, cb)
	...
	if not seconds then
		vim.schedule(function()
			cb(false, "Invalid time format: " .. tostring(time_spent))
		end)
		return
	end
	local mutation = [[
...
function M.statusline()
	if state.timer_start then
		...
		local scope = (M._effective_scope and M._effective_scope()) or (config.scope or "group")
		return string.format("⏱ %s%s", time_str, issue_part)
	else
		return ""
	end
end
